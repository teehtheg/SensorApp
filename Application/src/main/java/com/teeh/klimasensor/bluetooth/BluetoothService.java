...
public class BluetoothService {
    ...
    private class ConnectThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                // Close the socket
                Log.e(TAG,"Could not connect because of: " + e.getLocalizedMessage());
                try {
                    mmSocket.close();
    ...
    private class ConnectedThread extends Thread {
        ...
        private Integer currentPkg = null;
        private Integer totalPkgs = null;
            ...
            while (mState == STATE_CONNECTED) {
                try {
                    ...
                    if (str.contains(";")) {
                        ...
                        msgType = msgParts[0];
                        String msgHeader = msgParts[1];
                        msgContent = msgParts[2];


                        if (msgHeader.contains("/")) {
                            String[] msgHeaderParts = msgHeader.split("/");
                            currentPkg = Integer.valueOf(msgHeaderParts[0]);
                            totalPkgs = Integer.valueOf(msgHeaderParts[1]);
                            Log.i(TAG, "received pkg: " + currentPkg + "/" + totalPkgs);
                            String progress = currentPkg + "/" + totalPkgs;
                            mHandler.obtainMessage(Constants.MESSAGE_PROGRESS, progress)
                                    .sendToTarget();
                            isFile = true;
                        }
                    }
                    ...
                    if (isFile && currentPkg <= totalPkgs) {
                        ...
                        if (currentPkg.compareTo(totalPkgs) == 0) {
                            ...
                            if ("u".equals(msgType)) {
                                mHandler.obtainMessage(Constants.MESSAGE_UPDATE, file)
                                        .sendToTarget();
                            }
                            else if ("d".equals(msgType)) {
                                mHandler.obtainMessage(Constants.MESSAGE_FILE, file)
                                        .sendToTarget();
                            }
